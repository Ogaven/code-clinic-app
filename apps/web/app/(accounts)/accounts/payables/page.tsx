...
function QBConnect() {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-5 max-w-xs">Connect your QuickBooks account to view supplier balances.</p>
...
export default function SupplierBalancesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch('/api-proxy/accounts/quickbooks/vendors', { headers: { Authorization: `Bearer ${tok()}` } }).then(r => r.json())
      setVendors(res.data || [])
    } catch {}
    setLoading(false)
  }
  ...
  const filtered     = vendors.filter(v => !search || (v.DisplayName || '').toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 p-4 inline-block text-center">
        <p className="text-[10px] text-gray-400 uppercase font-semibold mb-1">Total Payable</p>
      ...
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-gray-400"><AlertCircle size={28} className="mb-2 opacity-40" /><p className="text-sm">No vendors found</p></div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-white/5">
              {filtered.map((v: any) => (
                <tr key={v.Id} className="hover:bg-blue-50/30 dark:hover:bg-white/5">
                  <td className="px-4 py-2.5 text-xs font-medium text-gray-800 dark:text-gray-200">{v.DisplayName}</td>
                  <td className="px-3 py-2.5 text-[11px] text-gray-500 hidden md:table-cell">{v.PrimaryEmailAddr?.Address || '—'}</td>
                  <td className="px-3 py-2.5 text-[11px] text-gray-500 hidden lg:table-cell">{v.PrimaryPhone?.FreeFormNumber || '—'}</td>
                  <td className="px-4 py-2.5 text-right text-xs font-bold text-red-500">{Number(v.Balance || 0).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
