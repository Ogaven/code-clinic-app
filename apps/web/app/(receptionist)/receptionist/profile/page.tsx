...
export default function ReceptionistProfilePage() {
  ...
  async function removeAvatar() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({}))
        showToast(err.error || 'Failed to remove photo', 'error')
      }
    } catch {
  ...
  async function handleProfileSave(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      // Self-scoped /auth/me works for every role (unlike the admin-only
      // /employees/:id, which 403s on a receptionist editing their own name).
      ...
      if (res.ok) {
        ...
        setUser(updated)
        localStorage.setItem('cc_user', JSON.stringify(updated))
        // ReceptionistLayout only reads localStorage once on mount — this
        // pushes the new name to the TopBar immediately, same pattern
  ...
  return (
    <div className="max-w-3xl mx-auto p-5 space-y-6 animate-fade-in">
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl font-semibold text-clinic-navy dark:text-white">My Profile</h2>
        <p className="text-sm text-gray-400 mt-0.5">Your personal photo, details and account security</p>
      </div>
      ...
      <form onSubmit={handleProfileSave} className="bg-white dark:bg-white/5 rounded-2xl shadow-sm border border-gray-100 dark:border-white/10 p-6 space-y-4">
        ...
        <div className="flex justify-end pt-2">
          <button type="submit" disabled={saving}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-white text-sm transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-60"
            style={{ background: 'linear-gradient(135deg,#0891b2,#06b6d4)', boxShadow: '0 4px 14px rgba(6,182,212,0.3)' }}>
