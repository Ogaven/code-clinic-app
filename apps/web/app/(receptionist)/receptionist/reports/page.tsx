...
import { BarChart2, Download, Bot, Loader2, Star, MessageSquare } from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0 bg-white dark:bg-[#0a1520] border-b border-gray-100 dark:border-white/8">
        ...
        <div className="flex gap-0.5 px-6 pt-3">
          {([
            { key: 'flow',     label: 'Patient Flow' },
            { key: 'feedback', label: 'Feedback'     },
          ] as { key: ReportsTab; label: string }[]).map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              {t.key === 'feedback' ? <Star size={13} /> : <BarChart2 size={13} />}
              {t.label}
            </button>
          ))}
        </div>
