...
function PatientFlowTab() {
  ...
  const [date, setDate]       = useState(() => new Date().toLocaleDateString('en-CA', { timeZone: 'Africa/Kampala' }))
  ...
  function exportFlowPDF() {
    const today = new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Africa/Kampala' })
    ...
    const rows = appts.map(a => {
      const t     = new Date(a.startAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Africa/Kampala' })
                ...
                {data.recent.map(r => (
                  <div key={r.id} className="flex items-start gap-4 px-5 py-4">
                    ...
                    <p className="text-[11px] text-gray-400 flex-shrink-0 whitespace-nowrap">
                      {new Date(r.submittedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', timeZone: 'Africa/Kampala' })}
                    </p>
                  </div>
                ))}
              </div>
