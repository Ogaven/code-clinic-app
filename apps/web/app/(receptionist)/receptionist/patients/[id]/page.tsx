...
function NotesTab({ patientId, patient }: { patientId: string; patient?: any }) {
  ...
  const handlePrint = () => {
    ...
    const notesHtml = notes.map(n => {
      const date   = new Date(n.createdAt).toLocaleString('en-UG', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })
      const author = n.author ? `${n.author.firstName} ${n.author.lastName}` : ''
      const body   = (n.content || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Notes — ${name}</title>
<style>
...
.logo{font-size:22px;font-weight:900;color:#1A237E}.logo span{color:#29ABE2}
...
.patient-info h2{font-size:16px;font-weight:700;margin:0 0 6px;color:#1A237E}
...
.footer{text-align:center;font-size:10px;color:#aaa;margin-top:32px;border-top:1px solid #e0e0e0;padding-top:12px}
</style></head><body>
<div class="header"><div class="logo">Code<span>Clinic</span> EMR</div><div class="subtitle">Patient Treatment Notes</div></div>
<div class="patient-info"><h2>${name}</h2><p>Patient ID: ${pid}</p><p>Date of Birth: ${dob}</p><p>Phone: ${phone}</p></div>
<div class="section-title">Treatment Notes (${notes.length})</div>
${notesHtml || '<p class="empty">No notes recorded for this patient.</p>'}
<div class="footer">Generated by Code Clinic EMR &bull; codeclinicemr.com &bull; ${new Date().toLocaleString('en-UG')}</div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        <button onClick={handlePrint}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-gray-600 dark:text-white/70 border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 transition-all">
