...
export default function PayrollPage() {
  ...
  function fetchPayroll(m = month) {
    setLoading(true)
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/payroll?month=${m}`, {
  ...
  async function runPayroll() {
    ...
    setRunning(true)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/payroll/${month}/run`, {
        method: 'POST',
  ...
  async function markPaid(id: string) {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/payroll/${id}/mark-paid`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${token}` },
    })
