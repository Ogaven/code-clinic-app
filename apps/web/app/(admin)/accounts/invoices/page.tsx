...
export default function InvoicesPage() {
  ...
  const fetch_ = useCallback(async (p = page, status = statusFilter) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/invoices?${params}`, {
  ...
  const openInvoice = async (inv: Invoice) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/invoices/${inv.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
...
function PaymentModal({ invoice, onClose, onPaid, token }: any) {
  ...
  async function pay() {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/payments`, {
        method: 'POST',
...
function CreateInvoiceModal({ onClose, onCreated, token }: any) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/patients?q=${patientSearch}&limit=5`, {
        headers: { Authorization: `Bearer ${token}` },
  ...
  async function submit() {
    ...
    setLoading(true); setError(null)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/invoices`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
