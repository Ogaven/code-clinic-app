...
export default function ExpensesPage() {
  ...
  function fetchExpenses(cat = filterCategory, month = monthFilter) {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/expenses?${params}`, {
  ...
  async function deleteExpense(id: string) {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/expenses/${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    })
...
function AddExpenseModal({ onClose, onAdded, token }: any) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/expenses`, {
        method: 'POST',
