...
import {
  ...
  CheckCircle, XCircle, Clock, ChevronRight, Users, FileText,
} from 'lucide-react'
...
async function downloadDoctorWordDoc(doc: CADoctor, monthLabel: string) {
  const {
  ...
  // Header
  children.push(new Paragraph({ text: `${doc.name} — Case Acceptance Report`, heading: HeadingLevel.HEADING_1, spacing: { after: 80 } }))
  children.push(new Paragraph({ text: `Code Clinic | ${monthLabel}`, heading: HeadingLevel.HEADING_2, spacing: { after: 80 } }))
  children.push(new Paragraph({
    children: [new TextRun({ text: `Generated: ${new Date().toLocaleDateString('en-GB')}`, color: '666666', size: 20 })],
    spacing: { after: 320 },
  }))

  // Stats table
  children.push(new Paragraph({ text: 'Summary', heading: HeadingLevel.HEADING_3, spacing: { before: 200, after: 160 } }))
  const statsRows = [
    new TableRow({
      tableHeader: true,
  ...
  if (totalValue > 0) {
    children.push(new Paragraph({
      children: [new TextRun({ text: `Total accepted value: ${fmtUGX(totalValue)}`, bold: true, color: '16a34a' })],
      spacing: { after: 240 },
    }))
  }

  function patientTable(label: string, rows: PatientEntry[], showValue: boolean) {
    if (rows.length === 0) return
    children.push(new Paragraph({ text: `${label} (${rows.length})`, heading: HeadingLevel.HEADING_3, spacing: { before: 200, after: 160 } }))
    const headers = showValue ? ['Patient', 'Service', 'Date', 'Value'] : ['Patient', 'Service', 'Date']
    children.push(new Table({
      width: { size: 100, type: WidthType.PERCENTAGE },
      rows: [
        new TableRow({
          tableHeader: true,
          children: headers.map(h => new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, size: 18 })] })], shading: { fill: 'E8F0FE' } })),
        }),
        ...rows.map(r => new TableRow({
          children: [
            r.name, r.service, fmtDate(r.date), ...(showValue ? [fmtUGX(r.value)] : []),
  ...
  patientTable('Accepted', doc.patients.accepted, true)
  patientTable('Pending / Follow-up', doc.patients.pending, false)
  patientTable('Declined', doc.patients.declined, false)
  ...
  a.href = url
  a.download = `case-acceptance-${doc.name.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-${monthLabel.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.docx`
  a.click()
      ...
      <div className="relative z-10 w-full sm:max-w-2xl max-h-[92dvh] flex flex-col bg-white dark:bg-[#0f1f2e] rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-white/10 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => downloadDoctorWordDoc(doc, monthLabel)}
