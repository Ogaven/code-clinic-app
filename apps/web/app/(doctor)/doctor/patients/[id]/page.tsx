...
function NotesTab({ patientId, token, patient }: { patientId: string; token: string | null; patient?: any }) {
...
export default function PatientProfilePage() {
  ...
  function switchTab(tab: ActiveTab) {
    setActiveTab(tab)
    router.replace(`/doctor/patients/${id}?tab=${tab}`, { scroll: false })
  }
  ...
  const renderTab = () => {
    switch (activeTab) {
      ...
      case 'notes': return <NotesTab patientId={id!} token={token} patient={patient} />
  ...
  return (
    <div className="space-y-4 animate-fade-in max-w-7xl">
      {/* Back */}
      <button onClick={() => router.push('/doctor/patients')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-clinic-navy dark:hover:text-white transition-colors">
