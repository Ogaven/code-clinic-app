...
import { ArrowLeft, Plus, Trash2, CalendarDays, Clock, Ban } from 'lucide-react'
...
export default function DoctorSchedulePage() {
  ...
  async function handleBlock() {
    ...
    try {
      ...
      if (res.ok) {
        setToast('Time blocked successfully!'); setTimeout(() => setToast(''), 3000)
        setBlockNote('')
        fetchAll()
      }
  ...
  return (
    <div className="space-y-5 p-4 md:p-6 animate-fade-in pb-24 md:pb-6">
      {toast && (
        <div className="fixed top-4 right-4 z-[99999] bg-emerald-600 text-white px-5 py-3 rounded-2xl shadow-2xl text-sm font-semibold">{toast}</div>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400 mt-0.5">Your appointments and blocked time</p>
        </div>
        <input type="date" value={viewDate} onChange={e => setViewDate(e.target.value)}
          className="text-sm bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>

      {/* ── SECTION A: My Calendar ──────────────────────────────────────── */}
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-50 dark:border-white/[0.06] flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-gray-800 dark:text-white">Appointments — {new Date(viewDate + 'T12:00').toLocaleDateString('en-UG', { weekday: 'long', day: 'numeric', month: 'long' })}</h2>
          </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-50 dark:divide-white/[0.04]">
            {appointments.sort((a,b) => new Date(a.startAt).getTime() - new Date(b.startAt).getTime()).map((a) => {
              const cfg = STATUS_CFG[a.status] || { label: a.status, color: '#6B7280', bg: '#F3F4F6' }
              return (
                <div key={a.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-blue-50/20 dark:hover:bg-white/[0.03] transition-colors">
                  <div className="w-16 text-right flex-shrink-0">
                    <p className="text-xs font-bold text-gray-700 dark:text-white">{fmtTime(a.startAt)}</p>
                    <p className="text-[10px] text-gray-400">{fmtTime(a.endAt || a.startAt)}</p>
                  </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer w-fit">
            <div onClick={() => setBlockAllDay(v => !v)}
              className={cn('w-11 h-6 rounded-full transition-colors relative', blockAllDay ? 'bg-blue-500' : 'bg-gray-200 dark:bg-white/20')}>
              <div className={cn('absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all', blockAllDay ? 'left-5.5' : 'left-0.5')} />
            </div>
          ...
          <button onClick={handleBlock} disabled={blockSaving}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm bg-orange-500 hover:bg-orange-600 text-white disabled:opacity-50 transition-colors min-h-[44px]">
