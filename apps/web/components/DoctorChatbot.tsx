...
const GREETINGS: Msg[] = [
  { from: 'sarah', text: "Hello Doctor! 👋 I'm Sarah, your personal AI assistant. How can I help you today?", time: now() },
  { from: 'sarah', text: "I can help with your schedule, patient records, treatment notes, billing queries, and support requests. Just ask! 🦷", time: now() },
]
...
export default function DoctorChatbot() {
  ...
  function handleClick() { if (!hasMoved) setOpen(o => !o) }
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      const res = await fetch('/api-proxy/assistant/chat', {
        ...
        body: JSON.stringify({ messages: newHistory, context: { page: window.location.pathname, role: 'DOCTOR' } }),
      })
      const data = await res.json()
      const reply = data.content || "I'm here to help! 🦷"
      setChatHistory(h => [...h, { role: 'assistant', content: reply }])
      setMsgs(m => [...m, { from: 'sarah', text: reply, time: now() }])
    } catch {
      setMsgs(m => [...m, { from: 'sarah', text: "Sorry, I couldn't connect right now. Please try again! 🙏", time: now() }])
    ...
    <div ref={bubbleRef} style={wrapStyle} onMouseDown={onMouseDown} onTouchStart={onTouchStart}>
      ...
      {open && !minimised && (
        <div className="mb-4 rounded-3xl shadow-2xl overflow-hidden animate-slide-right"
          style={{ width: '340px', background: 'linear-gradient(145deg,#0d1b6e,#1A237E)', border: '1px solid rgba(255,255,255,0.15)', cursor: 'default' }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-[10px] text-emerald-300">Doctor AI Assistant · Online</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <button onClick={() => setMin(true)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="rounded-2xl px-4 py-3" style={{ background: 'rgba(255,255,255,0.12)', borderRadius: '4px 16px 16px 16px' }}>
                  <div className="flex gap-1 items-center h-3">
                    {[0, 1, 2].map(i => <span key={i} className="w-1.5 h-1.5 rounded-full bg-blue-300 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}
                  </div>
                </div>
              </div>
            )}
      ...
      {open && minimised && (
        <div className="mb-4 flex items-center gap-3 px-4 py-2.5 rounded-2xl shadow-xl cursor-pointer"
          ...
          onClick={() => setMin(false)} onMouseDown={e => e.stopPropagation()}>
      ...
      {/* Floating Bubble */}
      <div onClick={handleClick} className={`select-none cursor-pointer ${!dragging && !open ? 'animate-float' : ''}`}
