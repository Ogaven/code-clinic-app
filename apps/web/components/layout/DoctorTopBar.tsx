...
export default function DoctorTopBar({ user, theme, onTheme, install }: { user: UserInfo | null; theme: AppTheme; onTheme: (theme: AppTheme, dark: boolean) => void; install?: PwaInstallState }) {
  ...
  return <>
    ...
    {notifOpen && <><button className="fixed inset-0 z-[90]" onClick={() => setNotifOpen(false)} /><div className="fixed right-4 top-[70px] z-[100] w-[min(380px,calc(100vw-2rem))] overflow-hidden rounded-2xl border bg-white shadow-2xl dark:border-white/10 dark:bg-[#0c1b38]"><div className="border-b px-4 py-3 text-sm dark:border-white/10 dark:text-white">Notifications</div><div className="max-h-[380px] overflow-y-auto">{notifications.length ? notifications.slice(0, 8).map(n => <Link key={n.id} href={n.href || '/doctor/notifications'} className="block border-b px-4 py-3 hover:bg-gray-50 dark:border-white/5 dark:hover:bg-white/5"><span className="block text-xs text-gray-800 dark:text-white">{n.title}</span><span className="line-clamp-2 text-[11px] text-gray-400">{n.body}</span></Link>) : <p className="py-10 text-center text-xs text-gray-400">No notifications yet</p>}</div></div></>}
