...
// Shared Daily/Weekly/Monthly Clinical Report implementation — reused by
// both Admin (apps/web/app/(admin)/reports/clinical/page.tsx, standalone
// page with a back link to /reports) and Receptionist (embedded as a tab
// inside apps/web/app/(receptionist)/receptionist/reports/page.tsx, no back
// link since it's not a standalone page there). Same GET /reports/clinical
// endpoint, same calculations, same drilldown, same exports — only the
// patient-link destination and the optional back link are role-aware.
...
export default function ClinicalReportBoard({ patientBasePath = '/patients', backHref }: {
  ...
  async function downloadWordDoc() {
    ...
    const {
      ...
      HeadingLevel, ImageRun, WidthType,
    } = await import('docx')
    ...
    try {
      ...
      children.push(new Paragraph({
        children: [new ImageRun({ data: logoData, transformation: { width: 160, height: 52 }, type: 'png' })],
        spacing: { after: 200 },
      }))
    } catch {}

    // Title & period
    children.push(new Paragraph({ text: 'Code Clinic — Clinical Report', heading: HeadingLevel.HEADING_1, spacing: { after: 80 } }))
    children.push(new Paragraph({ text: data.period.label, heading: HeadingLevel.HEADING_2, spacing: { after: 80 } }))
    children.push(new Paragraph({
      children: [new TextRun({ text: `Generated: ${new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}`, color: '666666', size: 20 })],
      spacing: { after: 320 },
    }))

    // Metrics table
    children.push(new Paragraph({ text: 'Clinical Metrics', heading: HeadingLevel.HEADING_3, spacing: { before: 200, after: 160 } }))
    const metricsRows = [
      new TableRow({
        tableHeader: true,
      ...
      ...STATS.map(s => new TableRow({
        children: [
          ...
          new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: String(s.value), bold: true })] })] }),
        ],
      })),
    ]
    children.push(new Table({ rows: metricsRows, width: { size: 60, type: WidthType.PERCENTAGE } }))
    ...
    if (data.followUpList.length > 0) {
      ...
      const followUpRows = [
        new TableRow({
          tableHeader: true,
          children: ['Patient', 'Phone', 'Appt Date', 'Service', 'Doctor', 'Reason', 'Days Since', 'Auto Sent'].map(h =>
            new TableCell({
        ...
        ...data.followUpList.map(r => new TableRow({
          children: [
            r.patientName,
            r.phone,
      ...
      children.push(new Table({ rows: followUpRows, width: { size: 100, type: WidthType.PERCENTAGE } }))
      children.push(new Paragraph({ text: '', spacing: { after: 320 } }))
    }

    // Footer
    children.push(new Paragraph({
      children: [new TextRun({ text: 'Generated by Code Clinic EMR', color: '999999', size: 18, italics: true })],
      spacing: { before: 400 },
    }))
      ...
      <div className="space-y-5 p-6 animate-fade-in">
        ...
        <div className="flex items-center gap-3 flex-wrap no-print">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={downloadWordDoc}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/10 transition-colors">
              <FileText size={14} /> Word Report
            </button>
                      ...
                      return (
                        <tr key={r.appointmentId}
                          ...
                          <td className="px-4 py-3">
                            <Link href={`${patientBasePath}/${r.patientId}`}
