...
// Extracted from what was previously the entire content of
// apps/web/app/(receptionist)/receptionist/appointments/page.tsx, unchanged,
// so it can be reused in two places without duplicating business logic:
//   1. The standalone /receptionist/appointments route (kept working exactly
//      as before — existing bookmarks/links/dashboard "View all" links
//      continue to resolve).
//   2. An "Appointments" tab inside /receptionist/scheduling, positioned to
//      match the Admin /scheduling page's own tab order (Calendar,
//      Appointments, Doctors, Services, Settings) — Admin's own
//      "Appointments" tab uses a separate AdminAppointmentsList component
//      (apps/web/components/scheduling/AdminAppointmentsList.tsx), which
//      that file's own comment explicitly says was forked FROM this
//      Receptionist page specifically so Admin's redesign wouldn't need to
//      touch Receptionist. This file is that same original real,
//      Receptionist-scoped implementation — same endpoints
//      (/scheduling/appointments, /scheduling/appointments/:id/status,
//      /scheduling/appointments/:id), same permissions (delete stays
//      admin-only via the existing isAdmin check below), nothing
//      backend-facing changed.

import { useEffect, useState, useRef } from 'react'
import { useRouter } from 'next/navigation'
import {
  Search, Download, Upload, Plus, X, Loader2, CalendarDays,
  ChevronLeft, ChevronRight, MoreHorizontal, XCircle,
  Phone, User,
} from 'lucide-react'
...
export default function ReceptionistAppointmentsList() {
