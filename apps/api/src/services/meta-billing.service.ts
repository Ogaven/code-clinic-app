...
interface CacheShape extends MetaBillingStatus { cachedAt: string }

function readCache(): CacheShape | null {
  try {
    const c = JSON.parse(fs.readFileSync(CACHE_FILE, 'utf-8')) as CacheShape
    if (Date.now() - new Date(c.cachedAt).getTime() < CACHE_TTL_MS) return c
    return null
  } catch { return null }
}

function writeCache(data: MetaBillingStatus): void {
...
async function buildBillingStatus(): Promise<MetaBillingStatus> {
  ...
  // what Meta itself returned in an error response.
  ...
  const billingStatus: BillingStatus = within24h
    ? 'ATTENTION_REQUIRED'
    : (graphApiError && creditLines.length === 0) ? 'UNKNOWN' : 'HEALTHY'

  return {
    billingStatus,
    wabaAccountReviewStatus: wabaStatus,
    creditLines,
    creditLinesSource,
    creditLinesNote,
    recent131042: recentFailure != null,
...
export async function getMetaBillingStatus(forceRefresh = false): Promise<MetaBillingStatus> {
  ...
  const data = await buildBillingStatus()
  writeCache(data)
  return data
}
